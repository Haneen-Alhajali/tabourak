// lib\content\availability&calender\schedulesTab\date_section\add_hours_modal.dart
...
class AddHoursModal extends StatefulWidget {
  final Map<DateTime, List<TimeRange>> initialDateSpecificHours;
  final String? scheduleId;
  final Function(DateTime)? onDeleteDate;

  const AddHoursModal({
    Key? key,
    required this.initialDateSpecificHours,
    this.scheduleId,
    this.onDeleteDate,
  }) : super(key: key);
...
class _AddHoursModalState extends State<AddHoursModal> {
  ...
  void initState() {
    super.initState();
    _dateTimeRanges.addAll(widget.initialDateSpecificHours);
  }
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      for (final date in _dateTimeRanges.keys) {
        final timeRanges = _dateTimeRanges[date]!;
        for (final range in timeRanges) {
          if (_isStartAfterEnd(range) || _isStartSameAsEnd(range)) {
            throw Exception('Invalid time range detected');
          }
        }
      }

      // Convert to format expected by backend
      final Map<String, List<Map<String, dynamic>>> dateSpecificHours = {};
      final datesToDelete = widget.initialDateSpecificHours.keys
          .where((date) => !_dateTimeRanges.containsKey(date))
          .map((date) => DateFormat('yyyy-MM-dd').format(date))
          .toList();

      _dateTimeRanges.forEach((date, ranges) {
        dateSpecificHours[DateFormat('yyyy-MM-dd').format(date)] = ranges.map((range) {
          return {
            'start': {
      ...
      final response = await http.put(
        Uri.parse('${AppConfig.baseUrl}/api/date-specific-availability'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(errorData['error'] ?? 'Failed to save changes');
      }
    } catch (e) {
      if (mounted) {
        ...
        SnackbarHelper.showError(context, 'Failed to save changes: ${e.toString()}');
  ...
  void _addTimeRange(DateTime date) {
    setState(() {
      ...
      // Don't add if the last slot is invalid
      if (_dateTimeRanges[date]!.isNotEmpty) {
        final lastSlot = _dateTimeRanges[date]!.last;
        if (_isStartAfterEnd(lastSlot) || _isStartSameAsEnd(lastSlot)) {
          return;
        }
      }
  ...
  void _removeTimeRange(DateTime date, int index) {
    setState(() {
      ...
      if (_dateTimeRanges[date]?.isEmpty ?? true) {
        ...
        if (widget.onDeleteDate != null) {
          widget.onDeleteDate!(date);
        }
      }
    });
  }
  ...
  void _removeAllDates() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete ALL date-specific hours?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
          ...
          TextButton(
            onPressed: () {
              ...
              if (widget.onDeleteDate != null) {
                for (final date in widget.initialDateSpecificHours.keys) {
                  widget.onDeleteDate!(date);
                }
              }
            },
  ...
  void _removeDate(DateTime date) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete ${DateFormat('MMMM d, y').format(date)}?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
...
//       final Map<String, List<Map<String, dynamic>>> dateSpecificHours = {};
//       _dateTimeRanges.forEach((date, ranges) {
//         dateSpecificHours[date.toIso8601String()] = ranges.map((range) {
//           return {
//             'start': {
//               'hour': range.start.hour,
//               'minute': range.start.minute,
