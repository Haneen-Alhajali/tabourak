...
exports.handleAppointments = async (req, res) => {
  try {
    ...
    // Get or create a default individual scheduling page for the member
    ...
    if (existingPages.length > 0) {
      pageId = existingPages[0].page_id;
    } else {
    ...
    // Get all existing appointments for this page
    const [existingAppointments] = await db.promise().query(
      `SELECT a.appointment_id, a.meeting_type 
       FROM appointments a
       WHERE a.page_id = ?`,
      [pageId]
    );
    ...
    if (typesToDeactivate.length > 0) {
      await db.promise().query(
        'UPDATE appointments SET is_active = FALSE WHERE page_id = ? AND meeting_type IN (?)',
        [pageId, typesToDeactivate]
      );
    }
      ...
      if (existingAppt) {
        ...
        const [apptResult] = await db.promise().query(
          `INSERT INTO appointments (
            organization_id, page_id, schedule_id, name, description,
        ...
        await db.promise().query(
          ...
          [apptResult.insertId, memberId, true]
        );
...
exports.getUserAppointments = async (req, res) => {
  try {
    ...
    // Get all individual scheduling pages for this member
    const [pages] = await db.promise().query(
      `SELECT page_id FROM scheduling_pages 
       WHERE organization_id = ? AND owner_member_id = ?`,
      [member[0].organization_id, member[0].member_id]
    );

    if (pages.length === 0) return res.json([]);

    // Get all appointments for these pages
    const [appointments] = await db.promise().query(
      `SELECT 
        ...
        a.meeting_type,
        a.is_active,
        a.page_color_primary as color_hex
      FROM appointments a
      WHERE a.page_id IN (?)`,
